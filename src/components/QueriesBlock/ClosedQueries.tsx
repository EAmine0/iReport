...
function Closed(props: Props) {
    ...
    if(listLabel.includes('Closed')){
        return(
            <>
            <div style={{width:"16%"}}>
             <Ant_Liquid percentage={(api[listLabel.indexOf('Closed')]?.value/api[listLabel.indexOf('Issued')]?.value)} /> Closed
            </div>
            </>
        )
    }
